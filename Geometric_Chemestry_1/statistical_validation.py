...
class StatisticalValidator:
    ...
    def calculate_distance_activity_correlation(self, embedding, pic50_values):
        """Calculate correlation between geometric distance and activity similarity"""
        print("Calculating distance-activity correlations...")
        ...
        for i in range(len(pic50_values)):
            for j in range(i+1, len(pic50_values)):
                activity_diffs.append(abs(pic50_values[i] - pic50_values[j]))
    ...
    def analyze_resonance_patterns(self, embedding, pic50_values, tolerance=0.1):
        """Analyze resonance patterns with statistical validation"""
        ...
        for name, target in resonance_targets.items():
            ...
            for i in range(len(embedding)):
                for j in range(i+1, len(embedding)):
                    ...
                    activity_diff = abs(pic50_values[i] - pic50_values[j])
                    
                    if abs(dist - target) < tolerance:
                        resonant_pairs.append(activity_diff)
                    else:
                        non_resonant_pairs.append(activity_diff)
    ...
    def create_visualizations(self, embedding_2d, embedding_3d, pic50_values, target_ids):
        ...
        sample_indices = np.random.choice(len(distances), sample_size, replace=False)
        
        axes[1, 1].scatter(distances[sample_indices], np.array(activity_diffs)[sample_indices], 
                          alpha=0.3, s=1)
...
def main():
    ...
    validator.create_visualizations(embedding_2d, embedding_3d, pic50_values, target_ids)
