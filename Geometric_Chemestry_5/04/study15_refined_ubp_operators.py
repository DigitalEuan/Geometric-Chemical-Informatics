...
def apply_ubp_operator(base_value, operator_type, C_rate, M_constant=math.e):
    ...
    # New: Combined operator for more nuanced control
    elif operator_type == 'combined_temp_sensitive':
        # This operator aims to accelerate reaction while also considering temperature stability.
        # It combines a linear acceleration with a dampening factor that increases with M_constant
        # and is more sensitive to temperature fluctuations (represented by C_rate indirectly affecting it).
        # The idea is to make the amplification less aggressive if temperature starts to rise too much.
        # For simplicity, let's make M_constant act as a dampening factor on the C_rate effect.
        # A higher M_constant means less amplification, potentially leading to better temperature control.
        return base_value * (1 + (C_rate / 100.0) / (1 + M_constant / 10.0))
    else:
        return base_value
...
# --- Study 15: Refined UBP Operators and Combinations ---
...
operator_types = [('linear', 'linear'), ('quadratic', 'quadratic'), ('compositional', 'compositional'), ('combined_temp_sensitive', 'combined_temp_sensitive')]

all_optimization_results = []

# Use a balanced weighting scheme for this study to focus on operator performance
weight_conc = 0.5
weight_temp = 0.5

for op_type_name, op_type_val in operator_types:
    x0 = [10.0, math.pi]  # Initial guess for [C_rate, M_constant]
    bounds = [(0.1, 500.0), (0.1, 50.0)] # Bounds for C_rate and M_constant

    print(f"\n--- Optimizing for UBP Operator: {op_type_name} (Balanced Weighting) ---")
    result = minimize(multi_objective_function, x0, args=(target_R, target_temp_std, op_type_val, initial_conditions, arrhenius_params, temp_params, Num_Steps, Time_Step_dt, weight_conc, weight_temp), bounds=bounds, method='L-BFGS-B')

    optimized_C_rate, optimized_M_constant = result.x
    final_r, final_p, final_t = simulate_reaction_full_output(optimized_C_rate, optimized_M_constant, op_type_val, initial_conditions, arrhenius_params, temp_params, Num_Steps, Time_Step_dt)

    all_optimization_results.append({
        'UBP_Operator_Type': op_type_name,
...
df_refined_optimization = pd.DataFrame(all_optimization_results)

output_path = "/home/ubuntu/ChemicalReactionKinetics/results/study15_refined_ubp_operators_results.csv"
df_refined_optimization.to_csv(output_path, index=False)
print(f"\nRefined UBP operator optimization results saved to {output_path}")

print("\nSample of Refined Operator Optimization Results:")
print(df_refined_optimization)
